...
__global__ void STReshapeRollVerticalKernel(float *pInput, int fea_b, int fea_h, int fea_w, int fea_c, int shift, float *pOutput)
{
    // B, H, W, C
...
__global__ void STReshapeRollHorizontalKernel(float *pInput, int fea_b, int fea_h, int fea_w, int fea_c, int shift, float *pOutput)
{
    const int index = blockIdx.x * 256 + threadIdx.x;
...
int32_t STReshapeRollPlugin::enqueue(const PluginTensorDesc *inputDesc, const PluginTensorDesc *outputDesc, const void *const *inputs, void *const *outputs, void *workspace, cudaStream_t stream) noexcept
{
    ...
    if (m.direction_ == 0){
        STReshapeRollVerticalKernel<<<grid, block, 0, stream>>>((float *)inputs[0], fea_b, fea_h, fea_w, fea_c, m.shift_, (float *)outputs[0]);
    }
    else if (m.direction_ == 1){
        STReshapeRollHorizontalKernel<<<grid, block, 0, stream>>>((float *)inputs[0], fea_b, fea_h, fea_w, fea_c, m.shift_, (float *)outputs[0]);
    }
    else{
        printf("No implement!");
    }
