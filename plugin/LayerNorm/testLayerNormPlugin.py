...
def layerNormCPU(bufferH, epsilon):
    _x = bufferH[0]
    _0 = np.mean(_x.reshape(_x.shape[0], _x.shape[1], -1), 2)[:, :, np.newaxis]
    _1 = _x - _0
    _2 = _1 * _1
    _3 = np.mean(_2.reshape(_x.shape[0], _x.shape[1], -1), 2)[:, :, np.newaxis]
    _4 = _3 + epsilon
    _5 = np.sqrt(_4)
    _6 = 1 / _5  # 1/sqrt(...)
    _7 = _1 * _6  # (x-μ)/sqrt(...)
...
if __name__ == '__main__':
    ...
    if os.path.isfile(trtFile):
        ...
        profile.set_shape('inputT', [nIn, qIn, eIn], [nIn*4, qIn, eIn], [nIn*4*4, qIn*2, eIn])
    ...
    context.set_binding_shape(0, [nIn, qIn, eIn])
    ...
    data = np.arange(nIn * qIn * eIn).reshape(nIn, qIn, eIn).astype(npDataType)
